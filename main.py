...
def get_db_path():
    # Store database inside app folder
    return os.path.join(App.get_running_app().user_data_dir, 'assets.db')
...
class DashboardScreen(Screen):
    ...
    def backup(self):
        try:
            src = get_db_path()
            dest = os.path.join(get_export_dir(), 'assets_backup.db')
    ...
    def export_csv(self):
        try:
            path = os.path.join(get_export_dir(), 'assets_export.csv')
...
class AssetsScreen(Screen):
    ...
    def load_assets(self, search='', category='All Categories'):
        ...
        for asset in c.fetchall():
            ...
            row.add_widget(Label(text=str(asset[1]), size_hint_x=0.28))
            row.add_widget(Label(text=str(asset[2] or '-'), size_hint_x=0.18))
            row.add_widget(Label(text=str(asset[3]), size_hint_x=0.1))
            row.add_widget(Label(text=f"{asset[4]:.0f}", size_hint_x=0.12))
            row.add_widget(Label(text=str(asset[5]), size_hint_x=0.12))
    ...
    def filter_assets(self):
        self.load_assets(self.ids.search.text, self.ids.category_spinner.text)
    ...
    def add_asset(self):
        ...
        for w in (name_input, category, quantity, value, notes):
        ...
        popup = Popup(title='Add Asset', content=content, size_hint=(0.9, 0.7))
        popup.open()
    ...
    def edit_asset(self, asset):
        ...
        def save(instance):
            conn = get_conn()
            c = conn.cursor()
            c.execute("""UPDATE assets SET name=?, category=?, quantity=?, value=?, notes=?, last_updated=?
                         WHERE id=?""",
                      (name_input.text.strip(), category.text, int(quantity.text or 1),
                       float(value.text or 0), notes.text.strip(),
                       datetime.now().isoformat(), asset[0]))
            conn.commit()
            conn.close()
            popup.dismiss()
            self.load_assets()
    ...
    def delete_asset(self, asset_id):
        ...
        content.add_widget(Label(text='Delete this asset?'))
        btn_layout = BoxLayout(spacing=10, size_hint_y=0.4)
        yes_btn = Button(text='Yes', background_color=(0.9, 0.2, 0.2, 1))
...
class EmployeesScreen(Screen):
    ...
    def load_employees(self):
        ...
        for emp in c.fetchall():
            ...
            row.add_widget(Label(text=str(emp[1]), size_hint_x=0.3))
            row.add_widget(Label(text=str(emp[2] or '-'), size_hint_x=0.25))
            row.add_widget(Label(text=str(emp[3] or '-'), size_hint_x=0.2))
    ...
    def add_employee(self):
        ...
        for w in (name, position, department, email):
            content.add_widget(w)
        ...
        btn_layout.add_widget(cancel_btn)
        content.add_widget(btn_layout)
        popup = Popup(title='Add Employee', content=content, size_hint=(0.9, 0.65))
        popup.open()
